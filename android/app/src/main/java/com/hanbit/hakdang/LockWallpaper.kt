...
object LockWallpaper {
    ...
    fun render(w: Word, width: Int, height: Int): Bitmap {
        ...
        val hunum = w.hunum.split("·").map { it.trim() }.filter { it.isNotEmpty() }.joinToString(" · ")

        // Measure everything first
        val tag = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = 0xFFE2B236.toInt(); textAlign = Paint.Align.CENTER; textSize = W * 0.03f
        }
        val box = if (chars.isEmpty()) 0f else min(W * 0.19f, (W * 0.8f) / chars.size)
        ...
        val hp = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = 0xFFCFE3DA.toInt(); textAlign = Paint.Align.CENTER; textSize = W * 0.042f
        }
        while (hp.measureText(hunum) > W * 0.86f && hp.textSize > 14f) hp.textSize *= 0.92f
        val tp = TextPaint(Paint.ANTI_ALIAS_FLAG).apply { color = 0xFFB8C6D0.toInt(); textSize = W * 0.037f }
        val en = StaticLayout.Builder.obtain(w.en, 0, w.en.length, tp, (W * 0.8f).toInt())
            .setAlignment(Layout.Alignment.ALIGN_CENTER).setMaxLines(2)
            .setEllipsize(android.text.TextUtils.TruncateAt.END).build()

        val gap = W * 0.03f
        val total = tag.textSize + gap + (if (box > 0) box + gap else 0f) + ko.textSize + gap +
            (if (hunum.isNotEmpty()) hp.textSize + gap * 0.6f else 0f) + en.height
        // Bottom of the block sits at 80% of the screen; never above the middle
        var y = maxOf(H * 0.5f, H * 0.8f - total)

        c.drawText(if (w.isNew) "오늘의 새 단어" else "오늘의 단어", cx, y + tag.textSize, tag)
        y += tag.textSize + gap
        ...
        if (box > 0) {
            ...
            chars.forEachIndexed { i, ch ->
                ...
                val fm = hz.fontMetrics
                c.drawText(ch.toString(), x0 + box / 2, y + box / 2 - (fm.ascent + fm.descent) / 2, hz)
            }
            y += box + gap
        }
        ...
        if (hunum.isNotEmpty()) {
            y += hp.textSize
            c.drawText(hunum, cx, y, hp)
            y += gap * 0.6f
        }

        c.save(); c.translate(W * 0.1f, y); en.draw(c); c.restore()
        return bmp
    }
}
