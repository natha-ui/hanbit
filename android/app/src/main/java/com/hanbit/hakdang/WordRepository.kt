...
class WordRepository(private val ctx: Context) {
    ...
    fun wordOfDay(): Word {
        ...
        if (prefs.getString("lockDate", "") == t) {
            prefs.getString("lockWord", null)?.let { s ->
                runCatching { return Word.from(JSONObject(s)).let { it.copy(hunum = cleanHunum(it.hunum)) } }
            }
        }
        val w = pick(t)
    ...
    private fun cleanHunum(h: String) = h.split("·").map { it.trim() }.filter { it.isNotEmpty() }.joinToString(" · ")
    ...
    private fun latestDaily(): List<Word> = runCatching {
        ...
        (0 until words.length()).map { i ->
            ...
            val hunum = (0 until chars.length())
                .map { j -> chars.getJSONObject(j).let { "${it.optString("hun")} ${it.optString("eum")}".trim() } }
